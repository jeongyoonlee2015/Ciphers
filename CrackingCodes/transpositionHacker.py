import pyperclip
from CrackingCodes import detectEnglish, transpositionDecrypt
...
def hackTransposition(message):
    print('Hacking...')
    print('(Press Ctrl-C(on Windows) or Ctrl-D (on MAC OS and Linux) to quit at any time.)')

    for key in range(1, len(message)):
        print('Traying key #%s...' % (key))
        ...
        if detectEnglish.isEnglish(decryptedText):
            ...
            print()
            print('Enter Dif done, anything else to continue hacking:')
            response = input('> ')
